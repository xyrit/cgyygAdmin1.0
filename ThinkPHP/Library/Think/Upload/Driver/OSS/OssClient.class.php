...
use Think\Upload\Driver\OSS\OssUtil;
use Think\Upload\Driver\OSS\OssException;
use Think\Upload\Driver\OSS\MimeTypes;
use Think\Upload\Driver\OSS\RequestCore;
use Think\Upload\Driver\OSS\ResponseCore;
use Think\Upload\Driver\OSS\PutSetDeleteResult;
...
class OssClient
{
     ...
     * @throws OssException
     */
    public function __construct($config, $isCName = false, $securityToken = NULL)
    {
        ...
        $this->hostname = $this->checkEndpoint($config['endpoint'], $isCName);
    ...
    public function downlink($key){
       ...
       // $key = urlencode($key);
        $key = self::Oss_escapeQuotes($key);
        $url = "http://".self::OSS_DOMAIN."/{$key}";
        return $url;

    }
    static function Oss_escapeQuotes($str){
        $find = array("\\", "\"");
        $replace = array("\\\\", "\\\"");
    ...
    //域名结果
    const OSS_DOMAIN = "img.cgyyg.com";  //
    //OSS ACL数组
