    ...
    public function update($data = null) {
        ...
        if (empty($data['id'])) { //新增数据
            ...
            if ($data['model_id'] == 5) {
                ...
                $pdata['lucky_code'] = $code;
                $pdata['create_time'] = $data['create_time'];
                $lid = M('lottery_product')->add($pdata);
                $updata['id'] = $lid;
                $updata['lottery_id'] = C('START_CODE') + $lid;
                M('lottery_product')->save($updata);
            }
        } else { //更新数据
            $status = $this->save(); //更新基础内容
            if ($data['id']) {
                $id = $data['id'];
                $data1 = M('lottery_product')->field('pid')->where("pid='$id'")->select();
                if ($data1) {
                    //如果商品存在则不添加到商品抽奖记录表
                } else {
                    //如果是新增商品，自动添加到商品抽奖记录表
                    if ($data['model_id'] == 5) {
                        $pdata['pid'] = $id;
                        $pdata['lottery_pid'] = 1;
                        $count = intval($data['price']);
