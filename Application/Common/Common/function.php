 ...
 * @param title 网页的标题
 * @param list 数据
 * @param field 查询的字段，如果取了别名，取别名后的字段
 * @param expCellName 在页面要显示的字段名
 */
function jerryExportExcel($title, $list, $field, $expCellName){
     vendor("PHPExcel");
...
//         $objWriter->save('php://output');
        ...
        //将excel保存到服务器
     $objWriter = \PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
     $fielName = $title.date('_YmdHis').'xls';//文件名可以根据需求改
     $dir = __ROOT__."/Public/Excel";//路径可以根据需求改
...
function get_location($id, $field = null)
{
    ...
    if (is_null($field)) {
        ...
        $pid = $list[$id]['pid'];
        $href_1 = U('Home/Article/index', array(
            'pid' => $id
        ));
        ...
        if ($pid > 0) {
            ...
            // 设置链接
            $href_2 = U('Home/Article/index', array(
                'pid' => $pid
            ));
            ...
            if (! empty($ThreeId)) { // 判断是否是一级分类,获取标题和标识
                ...
                // 设置链接
                $href_3 = U('Home/Article/index', array(
                    'pid' => $ThreeId
                ));
...
function addintocart($uid)
{
    ...
    function exportExcelJerry($title,$list,$field,$expCellName){
        vendor("PHPExcel");
        $objPHPExcel = new \PHPExcel();
        $objPHPExcel->setActiveSheetIndex(0);//把新创建的sheet设定为当前活动sheet，设置活动sheet，如果有3个sheet的话，是0，1，2
        $objSheet=$objPHPExcel->getActiveSheet();//获取当前活动sheet
...
function CheckSubstrs($substrs, $text)
{
     ...
     *
     */
    function exportExcelTest($title,$list,$field,$expCellName){
        vendor("PHPExcel");
        $objPHPExcel = new \PHPExcel();
        $objPHPExcel->setActiveSheetIndex(0);//把新创建的sheet设定为当前活动sheet，设置活动sheet，如果有3个sheet的话，是0，1，2
        $objSheet=$objPHPExcel->getActiveSheet();//获取当前活动sheet
